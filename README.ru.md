...
- **Поддержка HTTP/3 (QUIC)**: Специализированный раннер для анализа блокировок UDP-трафика.
- **Измерение скорости**: Встроенный тест пропускной способности для успешных стратегий обхода.
...
- **HTTP/3 (QUIC) Baseline**: (В полном профиле) Выполняет нативное QUIC-рукопожатие. Успех этого теста часто указывает на то, что сеть пропускает UDP-трафик, игнорируя стандартные правила DPI для TCP.
...
- `--speed`, `-s`: измерять скорость загрузки для успешных попыток.
